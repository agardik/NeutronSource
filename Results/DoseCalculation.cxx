...
public:
  DoseCalculation(const char *filename, const char *treename = "tree")
    ...
    //************************************************************************************//
    evt = 0;
    std::fill(std::begin(ParticleName), std::end(ParticleName), 0);
    ParentID = 0;
    ParticleID = 0;
    StepNumber = 0;
    posParticle[0] = 0.0;
  ...
  void Analyze(std::string histtitle, std::string histoutname,
    ...
    Double_t thickness = 0.2;                      // in cm, thickness of the slab
    Double_t mass =
        area * thickness * density * 1E-6; // in kg, 2 mm (0.2 cm) thickness,  Silicon
    ...
    for (long unsigned int j = 1; j < v_evt.size(); j++) {
      if (v_evt[j] == v_evt[j - 1]) {
        ...
        Hist_LET->Fill(edep_per_event / thickness); // in MeV/cm
    ...
    //************************************************************************************//
    double scale = 1; // scale factor to get per second rate
    ...
    Hist_LET->GetYaxis()->SetTitle("# Counts");
